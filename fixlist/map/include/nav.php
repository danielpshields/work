<h2>Nav</h2>
  <ul id="navUL">
    <div class="navItem">
      <div class="highlight_separate">
        <li><s>ACADEMICS</s></li>
        <ul>
          <li>Departments</li>
          <li>Centers and Institutes</li>
          <li>Programs</li>
          <li>Undergraduate</li>
          <ul>
            <li>Majors</li>
            <li class="red">Interdisciplinary Studies</li>
            <li>Study Abroad</li>
            <li>Exchange Programs</li>
            <li>Anderson Scholars</li>
          </ul>
          <li>Graduate</li>
          <li>International</li>
          <li>Curriculum Development</li>
        </ul>
      </div><!-- highlight separate -->
    </div><!-- nav item -->
    <div class="navItem">
      <div class="highlight_separate red">
        <li>ADMINISTRATION (DEAN'S OFFICE)</li>
        <ul>
          <li>People</li>
          <ul>
            <li>Meet The Dean</li>
            <li>Associate Deans</li>
            <li>Search Committees</li>
          </ul>
          <li><s>About</s></li>
          <ul>
            <li class="blue">Governance</a></li>
          </ul>
          <li><s>Offices</s></li>
          <ul>
            <li>Administrative Staff</li>
            <li>Budget And Finance</li>
            <li>Human Resources</li>
            <li>Recruitment</li>
            <li class="blue">CLAS IT</li>
            <li class="blue">Media Services</li>
            <li class="blue">Research Office</li>
            <li class="blue">Advancement Office</li>
            <li class="blue">Shared Service Center</li>
          </ul>
          <li>Dean’s Leadership Council</li>
          <li>Faculty Resources</li>
          <ul>
            <li>Faculty Research Funding And Travel</li>
            <li>Reporting</li>
            <li>Keene Faculty Center</li>
            <li>Publishing</li>
            <li>Tenure And Promotion</li>
          </ul>
          <li>Students</li>
          <ul>
            <li>Interdisciplinary Studies</li>
            <li>Student Research Funding And Travel</li>
            <li>Undergraduate Scholarships</li>
            <li>Graduate Memorial Scholarships</li>
            <li>O. Ruth McQuown Scholarship</li>
            <li>Cynthia W. Colangelo Award For Breast Cancer Awareness</li>
          </ul>
        </ul>
      </div><!-- highlight separate -->
    </div><!-- nav item -->

    <div class="navItem">
      <div class="highlight_separate">
        <li>ADVISING</li>
        <ul class="green">
          <li>Academic Advising Center</li>
          <li>Beyond120</li>
          <li class="white">Deans’ Medal</li>
          <li>Exploratory</li>
          <li>Pre-Health</li>
          <li>Pre-Law</li>
          <li>Transfer  </li>
        </ul>
      </div><!-- highlight separate -->
    </div><!-- nav item -->

    <div class="navItem">
      <li><s>ALUMNI</s></li>
      <ul>
        <li>Stories</li>
        <li class="blue">Giving</li>
        <li class="blue">Office of Advancement</li>
      </ul>
    </div><!-- nav item -->

    <div class="navItem">
      <li><s>NEWS AND EVENTS</s></li>
      <ul>
        <li class="orange">News</li>
        <li>Alumni Notes <i>(empty page)</i></li>
        <li>Awards & Recognition</li>
      </ul>
    </div><!-- nav item -->

    <div class="navItem">
      <li>RESEARCH</li>
      <ul>
        <li>Achievements</li>
        <li class="orange">Research News</li>
        <li>Undergraduate Research</li>
        <li>Faculty Research</li>
        <li class="blue">CLAS Research Office</li>
      </ul>
    </div><!-- nav item -->

      <div class="navItem">
        <li>ABOUT US</li>
        <ul>
          <li>Diversity and Inclusion</li>
          <li>College History <i>(broken link / empty page)</i></li>
          <li>Outreach</li>
          <li class="blue">Governance</li>
        </ul>
      </div><!-- nav item -->

</ul><!-- navUL -->
