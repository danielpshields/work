  <nav>
    <a href="index.php">
      <img src="http://www.clas.ufl.edu/prehealthpostbac/images/title2.jpg" border="0">
    </a>
    <ul>
      <li><a href="newstudents.php">Newly Admitted Students</a></li>
      <li class="white"><img src="http://www.clas.ufl.edu/prehealthpostbac/images/pointer_small.gif">Admission</li>
      <ul>
        <li class="small"><a class="noimage" href="admission-change.php">- Career Changers</a></li>
        <li class="small"><a class="noimage"  href="admission-restart.php">- ReStart Program</a></li>
      </ul>
      <li class="white"><img src="http://www.clas.ufl.edu/prehealthpostbac/images/pointer_small.gif"> Course Information</li>
      <ul>
        <li class="small"><a class="noimage"  href="courses-change.php">- Career Changers</a></li>
        <li class="small"><a class="noimage"  href="courses-restart.php">- ReStart Program</a></li>
      </ul>
      <li><a href="features.php">Program Features</a></li>
      <li><a href="finances.php">Financial Information</a></li>
      <li><a href="faq.php">FAQ</a></li>
      <li><a href="advising.php">Advising</a></li>
      <li><a href="alumnisuccess.php">Alumni Success</a></li>
      <li><a href="mailto:phpostbac@clas.ufl.edu">Contact Us</a></li>
    </ul>

  <!-- <h4>Search</h4>

  <form action="http://search.ufl.edu/search" method="get"><input type="hidden" name="site" value="clas.ufl.edu/">
  <label for="source">
  <select id="source" name="source">
  <option value="web" selected="selected">This Web Site</option>
  <option value="allweb">UF Web with Google</option>
  <option value="phonebook">UF Phonebook</option>
  </select>
  </label>
  <br />
  <label for="searchbox">
  <input name="query" id="searchbox" type="text" size="20" value="Search" alt="Search" onfocus="select();" />
  </label>
  <input type="submit" name="Submit" value="Go" />
  </form> -->

  <h4>Pre-Health Postbac Program (PHPB)</h4>
  <p>Farrior Hall<br>
  P.O Box 112015<br>
  Gainesville FL 32611<br>
  P: 352.273.4083<br>
  F: 352.392.2905<br>
  Email: <a href="mailto:phpostbac@clas.ufl.edu">phpostbac@clas.ufl.edu</a>
  </p>

  <p>
  <a href="https://www.facebook.com/UFPreHealthPostBac" class="socialmedia" title="UF Pre-Health Postbac Program on Facebook">
  <img alt="Facebook logo" src="images/fb.png" height="30px" width="30px" />
  </a>
  <a href="https://twitter.com/UFPHPB" class="socialmedia" title="UF PH PostBac">
  <img alt="Twitter logo" src="images/t.png" height="30px" width="30px" />
  </a>
  <a href="https://www.linkedin.com/groups?home=&gid=6774601&trk=anet_ug_hm" class="socialmedia" title="UF PH PostBac">
  <img alt="LinkedIn logo" src="images/linkedin.png" height="30px" width="30px" />
  </a>
  </p>
</nav><!-- end sidebar -->
