<?php
$title = "Pre-Health Post-Bac Completion Program | UF College of Liberal Arts and Sciences";
$page = "advising";
require_once("include/header.php");
?>

  <img class="mobileImage" src="image/advising.jpg" alt="">
  <img class="desktopImage" src="image/advising_desktop.jpg" alt="">

  <div class="page">

    <h2>Advising</h2>
    <p><b>If you have questions about the PHPB program please use walk-in hours in Farrior Hall as indicated below.</b></p>
    <table>
      <tr>
        <th>Monday</th>
        <th>Thursday</th>
      </tr>
      <tr>
        <td>8 a.m. to 11:30 a.m.</td>
        <td>8 a.m. to 11:30 a.m.</td>
      </tr>
    </table>

    <h3>Advising</h3>
    <p>Students will work closely with the program director to establish a professional development plan for building a competitive application. You will meet with the director at least once per semester to discuss and assess your plan and progress in PHPB.</p>
    <p>The director is also available Monday through Friday if you ever have questions that need to be answered. During your first semester you will take a pre-health seminar course to gain more in-depth knowledge about the profession. Professional development groups will encourage the knowledge and skills needed for application and interviews. The director will assist you step by step with what is needed for a competitive application.</p>
    <h3>Phone Advising</h3>
    <p>To speak with the PHPB Director please call 352-273-4083 to schedule a phone appointment.</p>
    <p><b>Students</b> may email at:  phpostbac@clas.ufl.edu</p>
    <p><b>Walk-in</b> and <b>phone advising</b> are generally for advising questions of 15 minutes or less. If you have numerous or lengthy questions please schedule an appointment.</p>
    <h3>Appointments</h3>
    <p>Call (352) 273-4083 to schedule an appointment.</p>
    <p>Please see our <a href="https://www.advising.ufl.edu/pre-health/" target="_blank">Pre-Health Advising website</a> for more information on preparing for health professional graduate programs.</p>



  </div><!-- page -->

<?php require_once("include/footer.php"); ?>
